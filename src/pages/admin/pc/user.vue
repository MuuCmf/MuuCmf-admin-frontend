  ...
  <div class="page-container">
    ...
    <div v-loading="loading" class="page-content">
      <div class="with-container">
        <div class="user-nav-config-wrapper">
          <div class="user-nav-list">
            <el-empty v-if="userNavList.length === 0 && !loading" description="暂无导航数据" />

            <div v-for="(item, index) in userNavList" :key="item.id" class="user-nav-item">
              <div class="item-header">
                ...
                <div class="item-actions">
                  ...
                  <el-button
                    ...
                    text
                    :icon="ArrowDown"
                    :disabled="index === userNavList.length - 1"
...
const handleDelete = (index: number) => {
  ElMessageBox.confirm('确定要删除该导航吗？', '提示', {
    ...
    .then(() => {
      userNavList.value.splice(index, 1);
...
const handleMoveUp = (index: number) => {
  if (index > 0) {
    const temp = userNavList.value[index];
    userNavList.value[index] = userNavList.value[index - 1];
    userNavList.value[index - 1] = temp;
    userNavList.value[index].sort = index;
    userNavList.value[index - 1].sort = index - 1;
  }
};

const handleMoveDown = (index: number) => {
  if (index < userNavList.value.length - 1) {
    const temp = userNavList.value[index];
    userNavList.value[index] = userNavList.value[index + 1];
    userNavList.value[index + 1] = temp;
    userNavList.value[index].sort = index;
...
const handleSubmit = async () => {
  if (userNavList.value.length === 0) {
    ElMessage.error('导航至少存在一个');
    return;
  }

  const hasEmptyTitle = userNavList.value.some(item => !item.title);
  if (hasEmptyTitle) {
  ...
  try {
    userNavList.value.forEach((item, index) => {
      item.sort = index;
    });

    const nav = {
      type: userNavList.value.map(item => item.type),
      app: userNavList.value.map(item => item.app),
      title: userNavList.value.map(item => item.title),
      url: userNavList.value.map(item => item.url),
      sort: userNavList.value.map((item, index) => index),
