      ...
      <div class="with-container">
        <div class="store-config-container">
          ...
          <div class="store-basic-config">
            <h3 class="config-section-title">短信基本配置</h3>
            <el-form ref="formRef" :model="formData" label-width="150px" label-position="right">
              <el-form-item v-for="item in smsConfigList" :key="item.id" :label="item.title">
                <component
          ...
          <div class="cloud-config-tabs">
            <el-tabs v-model="activeTab" @tab-change="handleTabChange">
              <el-tab-pane label="阿里云短信" name="aliyun">
                <div class="cloud-config-content">
                  <el-form :model="formData" label-width="150px" label-position="right">
                    <el-form-item v-for="item in aliyunSmsConfigList" :key="item.id" :label="item.title">
                      <component
                        :is="getConfigComponent(item.type)"
                        v-model="formData[item.name]"
                        :options="parseOptions(item.extra)"
              ...
              <el-tab-pane label="腾讯云短信" name="tencent">
                <div class="cloud-config-content">
                  <el-form :model="formData" label-width="150px" label-position="right">
                    <el-form-item v-for="item in tencentSmsConfigList" :key="item.id" :label="item.title">
                      <component
                        :is="getConfigComponent(item.type)"
                        v-model="formData[item.name]"
                        :options="parseOptions(item.extra)"
...
import { getExtendList, saveExtendSms } from '@/api/admin/extend';
import {
  Num,
  String,
  Textarea,
...
// 短信相关配置分组
const groupParams = 'sms,aliyun_sms,tencent_sms';
...
// 筛选出短信基本配置项
const smsConfigList = computed(() => {
  return configList.value.filter(item => item.group === 'sms');
});

// 筛选出阿里云短信配置项
const aliyunSmsConfigList = computed(() => {
  return configList.value.filter(item => item.group === 'aliyun_sms');
});

// 筛选出腾讯云短信配置项
const tencentSmsConfigList = computed(() => {
  return configList.value.filter(item => item.group === 'tencent_sms');
});
...
const handleSubmit = async () => {
  ...
  try {
    const res = await saveExtendSms(formData);
